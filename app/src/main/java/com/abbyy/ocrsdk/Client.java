...
public class Client {
	...
	// noduritoto
	public Task testProcessImage(Bitmap bitmapImage, ProcessingSettings settings)
			throws Exception {
		URL url = new URL(serverUrl + "/processImage?" + settings.asUrlParams());
		byte[] fileContents = readDataFromBitmap(bitmapImage);
	...
	// image to bitmap by noduritoto
	private byte[] readDataFromBitmap(Bitmap bitmapImage){
